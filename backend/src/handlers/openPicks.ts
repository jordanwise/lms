...
export async function handler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  try {
    ...
    if (!canTransition(game.state, game.roundState, 'OPEN_PICKS')) {
      return conflict(`Cannot open picks in state: ${compositeState(game.state, game.roundState)}`);
    }

    const newState = applyTransition(game.state, game.roundState, 'OPEN_PICKS');
    ...
    console.error('openPicks error:', err);
    return serverError();
  }
}
